...
def subPlotPar():
    plt.figure( constrained_layout=True, figsize=(10, 4.5))
    plt.xlabel('Sampling point', fontsize=20  )
    plt.ylabel('Trace number', fontsize=20)
    plt.xlim( 500, 3600 )
...
def pick(points, dirsave, nametag, nsta,order, pp, ptraveltimes, straveltimes,
         step, win, kurwindow, sr, dstop,
         vfilter, h1filter, h2filter, vfilter2, h1filter2, h2filter2,
         savefigure, overlap):
    vertical = vfilter
    horizon1 = h1filter
    horizon2 = h2filter
    ...
    for i,nm in zip(order,pp):
        ...
        number=number+1
        print('Doing phase picking for event (using 1st frequency band): '+str(number))
        #plt.clf()
        k=0
        onp=[]
        ons1=[]
            ...
            if savefigure == 'on':
                if onsetflag >= 0:
                    if pmarks[k-1][int(round(arrive * sr))] == 0:
                        plt.plot(a/max(abs(a))+2*k,  marker='o',
                                 ls='-', lw=1, color='black', markeredgecolor='black',
                                 ms=5, markevery=[potential[onsetflag]])
                    else:
                        plt.plot(a/max(abs(a))+2*k,  marker='o',
                                 ls='-', lw=1, color='black', markeredgecolor='red',
                                 ms=5, markevery=[potential[onsetflag]])
                else:
                    plt.plot(a/ max(abs(a)) + 2 * k,  ls='-', lw=1, color='black' )
        ...
        if savefigure == 'on':
            plt.savefig(dirsave+str(i[1])+'p_' + nametag + '.png')
            #plt.savefig(dirsave+str(i[1])+'p_' + nametag + '.svg')
            plt.close('all')
        ...
        if savefigure == 'on':   
            plt.savefig(dirsave + str(i[1]) + 's1_' +nametag + '.png')
            #plt.savefig(dirsave + str(i[1]) + 's1_' +nametag + '.svg')
            plt.close('all')
            ...
            if savefigure == 'on':
                if onsetflag >= 0:
                    if s2marks[k - 1][int(round(arrive * sr))] == 0:
                        ...
                        plt.plot(a / max(abs(a)) + 2 * k,  marker='o',
                                 ls='-', lw=1, color='black', markeredgecolor='red', ms=5,
        ...
        if savefigure == 'on':
            plt.savefig(dirsave + str(i[1]) + 's2_' + nametag + '.png')
            #plt.savefig(dirsave + str(i[1]) + 's2_' + nametag + '.svg')
            plt.close('all')
    ...
    for i, nm in zip(order, pp):
        ...
        number = number + 1
        print('Doing phase picking for event (using 2nd frequency band): '+str(number))
        #plt.clf()
        k = 0
        onp = []
        ons1 = []
        ...
        if savefigure == 'on':
            plt.savefig(dirsave + str(i[1]) + 'p_low_' + nametag + '.png')
            #plt.savefig(dirsave + str(i[1]) + 'p_low_' + nametag + '.svg')
            plt.close('all')
        ...
        if savefigure == 'on':
            ...
            plt.savefig(dirsave + str(i[1]) + 's1_low_' + nametag + '.png')
            #plt.savefig(dirsave + str(i[1]) + 's1_low_' + nametag + '.svg')
            plt.close('all')
        ...
        if savefigure == 'on':
            plt.savefig(dirsave + str(i[1]) + 's2_low_' + nametag + '.png')
            #plt.savefig(dirsave + str(i[1]) + 's2_low_' + nametag + '.svg')
            plt.close('all')
