...
def MAXI_locate(ponsets, sonsets, ptraveltimes, straveltimes, stlas, stlos, stz,
                lowq, highq, studygrids, Q_threshold, terr, outlier, tableP, tableS,
    ...
    for i in range(0,total):
        ...
        for wei in p:
            print( '\t', 'studygrids[wei] =', studygrids[wei] )
        ...
        for l in range(0, len(refinetraveldis)):
            ...
            for j in range(0,len(refinetraveldis[l])):
                time=taupz.taupz(tableP=tableP,tableS=tableS,dep=refinestudygrids[l][2], dis=refinetraveldis[l][j]
                                 ,phase='P',al=stz[j], disgrid=recDisScanInc, depgrid=srcDepthScanInc)
                a.append(time)
            ...
            for j in range(0,len(refinetraveldis[l])):
                time=taupz.taupz(tableP=tableP,tableS=tableS,dep=refinestudygrids[l][2], dis=refinetraveldis[l][j]
                                 ,phase='S',al=stz[j], disgrid=recDisScanInc, depgrid=srcDepthScanInc)
                b.append(time)
    ...
    ########################################################################################################################
        ...
        while refinedepgrid > mindepgrid:
            ...
            for l in range(0, len(refinetraveldis)):
                a = []
                for j in range(0, len(refinetraveldis[l])):
                    time = taupz.taupz(tableP=tableP,tableS=tableS, dep=refinestudygrids[l][2], \
                                       dis=refinetraveldis[l][j],phase='P', al=stz[j], disgrid=recDisScanInc, depgrid=srcDepthScanInc)
                    a.append(time)
                ...
                for j in range(0, len(refinetraveldis[l])):
                    time=taupz.taupz(tableP=tableP,tableS=tableS, dep=refinestudygrids[l][2], 
                                     dis=refinetraveldis[l][j], phase='S', al=stz[j], disgrid=recDisScanInc, depgrid=srcDepthScanInc)
                    b.append(time)
